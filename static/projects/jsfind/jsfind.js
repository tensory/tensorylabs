...
var JSFind = (function() {
	...
	function getContent() {
		return $(finder.options.textSelector);
	};
	...
	*/
	...
	finder.highlightAll = function(query) {
		...
		var bodyText = $(finder.options.textSelector)[0].innerHTML;

		for (var i = 0; i < queryArray.length; i++) {
			bodyText = tagAll(bodyText, queryArray[i]);
		}

		$(finder.options.textSelector)[0].innerHTML = bodyText;
		return true;
	};
	...
	*/
	 ...
	 */
	finder.init = function(options) {
		finder.options.textSelector = options.text || 'body';
		var showAtStart = options.show || false;
